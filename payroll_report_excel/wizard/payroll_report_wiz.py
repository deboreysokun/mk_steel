...
class payrollreportexcelwiz(models.TransientModel):
    ...
    def get_item_data(self):
        ...
        if self.from_date and self.date_end:
            ...
            date_2 = datetime.strftime(self.date_end, '%d-%b-%Y')
            date_1= datetime.strftime(self.from_date, '%d-%b-%Y')
            ...
            worksheet.write(row, 2, 'Employee', cell_text_format)
            worksheet.write(row, 3, 'Position', cell_text_format)
            worksheet.write(row, 4, 'Entry Date', cell_text_format)
            worksheet.write(row, 5, 'Entry Salary', cell_text_format)
            ...
            if payslip_ids:
                ...
                for payslip in payslip_ids:
                    ...
                    position = payslip.employee_id.job_id.name
                    entry_date = datetime.strftime(payslip.contract_id.date_start, '%d-%b-%Y')
                    entry_salary = payslip.contract_id.entry_salary
                    employee_number = payslip.employee_id.employee_number
                    line_index += 1


                    worksheet.write(ro, col, line_index or '', cell_text_format_new)
                    worksheet.write(ro, col + 1, employee_number or '', cell_text_format_new)
                    worksheet.write(ro, col + 2, name or '', cell_text_format_new)
                    worksheet.write(ro, col + 3, position or '', cell_text_format_new)
                    worksheet.write(ro, col + 4, entry_date or '', cell_text_format_new)
                    worksheet.write(ro, col + 5, entry_salary or '', cell_text_format_new)
